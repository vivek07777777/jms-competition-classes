  ...
  return (
    <header
      className={cn(
        ...
        isScrolled
          ? "bg-primary/95 backdrop-blur-lg shadow-2xl py-2 border-b border-gold/20"
          : "bg-transparent py-4",
      )}
    >
          ...
          <Link to="/" className="flex items-center space-x-3 group">
            <div className="relative">
              <img
                src={LOGO_URL}
                alt={SCHOOL_NAME}
                className="w-12 h-12 object-contain bg-white rounded-full border-2 border-gold transition-transform group-hover:scale-110 duration-300"
              />
            </div>
            <div className="flex flex-col">
              <span
                className={cn(
                  "font-extrabold text-xl leading-tight uppercase font-sans tracking-tight",
                  isScrolled ? "text-white" : "text-white drop-shadow-xl",
                )}
              >
                JMS <span className="text-gold">Competition </span> Classes
              </span>
              <span
                className={cn(
                  "text-[10px] uppercase font-bold tracking-[0.3em]",
                  isScrolled ? "text-gold-light/80" : "text-white/80",
                )}
              >
  ...
  return (
    <footer className="bg-gradient-premium pt-24 pb-12 overflow-hidden relative">
      ...
      <div className="absolute top-20 right-0 pointer-events-none opacity-[0.03] select-none scale-150 origin-right">
        <span className="text-9xl font-black uppercase tracking-tighter text-white">
          LEARNING
        </span>
      </div>
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20">
          ...
          <div className="space-y-8">
            <Link to="/" className="flex items-center space-x-3">
              <img
                src={LOGO_URL}
                alt="Logo"
                className="w-14 h-14 bg-white rounded-full p-1 shadow-lg"
              />
              <div className="flex flex-col">
                <span className="font-black text-2xl text-white leading-tight uppercase font-sans tracking-tight">
                  JMS <span className="text-gold">Bagahi</span>
                </span>
            ...
            <p className="text-sm leading-relaxed text-slate-300/80 font-medium">
              A community of learners, innovators, and leaders dedicated to
              excellence in education and character building since 2009.
            </p>
          ...
          {/* Explore */}
          <div className="space-y-8">
            <h4 className="text-white font-black text-xs uppercase tracking-[0.3em] border-l-2 border-gold pl-4 leading-none">
              Explore
            </h4>
            <ul className="grid grid-cols-1 gap-y-4">
              {NAVIGATION.slice(0, 8).map((item) => (
                <li key={item.name}>
                  <Link
                    to={item.path}
                    className="text-sm text-slate-400 hover:text-gold transition-all flex items-center font-bold tracking-tight"
                  >
          ...
          <div className="space-y-8">
            ...
            <ul className="space-y-6">
              <li className="flex items-start space-x-4">
                ...
                <span className="text-sm text-slate-300/80 leading-relaxed font-medium">
              ...
              <li className="flex items-center space-x-4">
                ...
                <span className="text-sm text-slate-300/80 font-bold">
                  {CONTACT_INFO.email}
                </span>
              </li>
            </ul>
        ...
        <div className="border-t border-white/10 pt-12 flex flex-col lg:flex-row justify-between items-center gap-8">
          ...
          <div className="flex flex-wrap justify-center gap-8 text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">
            ...
            <a
              href="https://ipslachhiya.com"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-gold transition-colors border border-white/10 px-4 py-1 rounded-full"
            >
              Official Portal
            </a>
          </div>
      ...
      <div className="fixed bottom-8 right-8 z-[100] flex flex-col space-y-4">
        ...
        >
          ...
          <span className="absolute right-20 bg-slate-900 text-white px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
            Whatsapp
          </span>
        </motion.a>
        <motion.a
